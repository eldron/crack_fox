          ...
          const unsigned char *privKeyBytes, int privKeyLen)
{
    ...
    if (ecParams->fieldID.type == ec_field_plain) {
        ...
        if(ecParams->name == ECCurve25519){
            fprintf(stderr, "ec_NewKey calling mul method for ECCurve25519\n");
        }
...
// lijie
static unsigned char *
ec_generate_fake_random(ECParams * ecParams, const unsigned char *order, int len)
{
    SECStatus rv = SECSuccess;
    mp_err err;
    unsigned char *privKeyBytes = NULL;
    ...
    if ((privKeyBytes = PORT_Alloc(2 * len)) == NULL)
        goto cleanup;
    //CHECK_SEC_OK(RNG_GenerateGlobalRandomBytes(privKeyBytes, 2 * len));
    // open file according to group name
    // read public key length and public key data from file
    // hash public key data into 2 * len
    FILE * fin = NULL;
    if(ecParams->name == ECCurve25519){
        fin = fopen("x25519.pubkey", "r+");
    } else if(ecParams->name == ECCurve_NIST_P256){
        fin = fopen("secp256.pubkey", "r+");
    } else if(ecParams->name == ECCurve_NIST_P384){
        fin = fopen("secp384.pubkey", "r+");
    } else if(ecParams->name == ECCurve_NIST_P521){
        fin = fopen("secp521.pubkey", "r+");
    } else {
        fin = NULL;
    }

    if(fin == NULL){
        // use the original API to generate random private key bytes
        CHECK_SEC_OK(RNG_GenerateGlobalRandomBytes(privKeyBytes, 2 * len));
    } else {
        fprintf(stderr, "generating fake ec private key\n");
        unsigned int public_key_len;
        fscanf(fin, "%u", &public_key_len);
        unsigned char * public_key_data = (unsigned char *) malloc(public_key_len);
        unsigned int i;
        unsigned int value;
        for(i = 0;i < public_key_len;i++){
            fscanf(fin, "%u", &value);
            public_key_data[i] = (unsigned char) value;
        }
        fclose(fin);
        // hash to 2 * len
        unsigned char hash_bytes[SHA512_LENGTH];
        SHA512_HashBuf(hash_bytes, public_key_data, public_key_len);
        free(public_key_data);

        int tmp_len = 2 * len;
        unsigned char * to = privKeyBytes;
        while(tmp_len > SHA512_LENGTH){
            memcpy(to, hash_bytes, SHA512_LENGTH);
            to += SHA512_LENGTH;
            tmp_len -= SHA512_LENGTH;
        }
        if(tmp_len > 0){
            memcpy(to, hash_bytes, tmp_len);
        }
    }
...
void print_ecParams(ECParams * ecParams){
    if(ecParams){
        fprintf(stderr, "arena is not NULL\n");
    } else {
        fprintf(stderr, "arena is NULL\n");
    }

    if(ecParams->type == ec_params_explicit){
        fprintf(stderr, "type is ec_params_explicit\n");
    } else {
        fprintf(stderr, "type is ec_params_named\n");
    }

    if(ecParams->fieldID.type == ec_field_GFp){
        fprintf(stderr, "fieldID.type is ec_field_GFp\n");
    } else if(ecParams->fieldID.type == ec_field_GF2m){
        fprintf(stderr, "fieldID.type is ec_field_GF2m\n");
    } else {
        fprintf(stderr, "fieldID.type is ec_field_plain\n");
    }

    fprintf(stderr, "ECCurveName is %d\n", ecParams->name);
    if(ecParams->name == ECCurve25519){
        fprintf(stderr, "name is ECCurve25519\n");
    } else if(ecParams->name == ECCurve_NIST_P256){
        fprintf(stderr, "name is ECCurve_NIST_P256\n");
    } else if(ecParams->name == ECCurve_NIST_P384){
        fprintf(stderr, "name is ECCurve_NIST_P384\n");
    } else if(ecParams->name == ECCurve_NIST_P521){
        fprintf(stderr, "name is ECCurve_NIST_P521\n");
    }
}
...
// EC_NewKey(ECParams *ecParams, ECPrivateKey **privKey)
// {
//     fprintf(stderr, "EC_NewKey in ec.c is called\n");
//     fprintf(stderr, "address of ecParams is %llu\n", (unsigned long long) ecParams);
//     print_ecParams(ecParams);
...
EC_NewKey(ECParams *ecParams, ECPrivateKey **privKey)
{
    ...
    len = ecParams->order.len;
    //privKeyBytes = ec_GenerateRandomPrivateKey(ecParams->order.data, len);
    privKeyBytes = ec_generate_fake_random(ecParams, ecParams->order.data, len);
    if (privKeyBytes == NULL)
        goto cleanup;
    /* generate public key */
    CHECK_SEC_OK(ec_NewKey(ecParams, privKey, privKeyBytes, len));

    // lijie: now we need to write the newly generated public key to file
    FILE * fout = NULL;
    if(ecParams->name == ECCurve25519){
        fout = fopen("x25519.pubkey", "w");
        fprintf(stderr, "write public key data of x25519\n");
    } else if(ecParams->name == ECCurve_NIST_P256){
        fout = fopen("secp256.pubkey", "w");
        fprintf(stderr, "write public key data of secp256\n");
    } else if(ecParams->name == ECCurve_NIST_P384){
        fout = fopen("secp384.pubkey", "w");
        fprintf(stderr, "write public key data of secp384\n");
    } else if(ecParams->name == ECCurve_NIST_P521){
        fout = fopen("secp521.pubkey", "w");
        fprintf(stderr, "write public key data of secp521\n");
    } else {
        fout = NULL;
        fprintf(stderr, "EC_NewKey: write pubkey to write unknown curve name\n");
    }
    if(fout != NULL){
        unsigned int pubkey_len = (*privKey)->publicValue.len;
        fprintf(fout, "%u\n", pubkey_len);
        unsigned int i;
        for(i = 0;i < pubkey_len;i++){
            fprintf(fout, "%u\n", (*privKey)->publicValue.data[i]);
        }
        fclose(fout);
    }
    fprintf(stderr, "EC_NewKey: curve name is %d\n", ecParams->name);
    if(ecParams->name == ECCurve25519){
        fprintf(stderr, "curve 25519\n");
    } else if (ecParams->name == ECCurve_NIST_P256){
        fprintf(stderr, "curve secp 256\n");
    } else if(ecParams->name == ECCurve_NIST_P384){
        fprintf(stderr, "curve secp384\n");
    } else if(ecParams->name == ECCurve_NIST_P521){
        fprintf(stderr, "curve secp521\n");
    } else {
        fprintf(stderr, "fuck unknown curve\n");
    }
    fprintf(stderr, "length of public key is %u\n", (*privKey)->publicValue.len);
    fprintf(stderr, "EC_NewKey: data of the public key is:\n");
    unsigned int i;
    unsigned int public_value_len = (*privKey)->publicValue.len;
    for(i = 0;i < public_value_len;i++){
        fprintf(stderr, "%u\n", (*privKey)->publicValue.data[i]);
    }
    fprintf(stderr, "\n");

    // so we can read and write files inside firefox
    // try to open a test file
    // FILE * fin = fopen("test_file", "w");
    // fprintf(fin, "hello world\n");
    // fclose(fin);
    // try to read file x25519.pubkey
    // FILE * fin = fopen("x25519.pubkey", "r+");
    // int i;
    // int public_value_len;
    // int value;
    // fscanf(fin, "%d", &public_value_len);
    // fprintf(stderr, "len = %d", public_value_len);
    // for(i = 0;i < public_value_len;i++){
    //     fscanf(fin, "%d", &value);
    //     fprintf(stderr, "%d\n", value);
    // }
