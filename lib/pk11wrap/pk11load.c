...
{
    fprintf(stderr, "secmod_LoadPKCS11Module is called\n");
    ...
    if (mod->internal && (mod->dllName == NULL)) {
        ...
        fprintf(stderr, "entry is set to NSC_GetFunctionList\n");
        ...
        if (mod->isFIPS) {
            ...
            fprintf(stderr, "entry is set to PR_FindSymbol softokenLib, FC_GetFunctionList\n");
        } else {
            entry = (CK_C_GetFunctionList)
                PR_FindSymbol(softokenLib, "NSC_GetFunctionList");

            fprintf(stderr, "entry is set to PR_FindSymbol softokenLib, NSC_GetFunctionList\n");
        }
        ...
        if (!mod->moduleDBOnly) {
            ...
            fprintf(stderr, "entry is set to PR_FindSymbol library, C_GetFunctionList\n");
        }
        if (mod->isModuleDB) {
            mod->moduleDBFunc = (void *)
                PR_FindSymbol(library, "NSS_ReturnModuleSpecData");

            fprintf(stderr, "entry is set to PR_FindSymbol library, NSS_ReturnModuleSpecData\n");
        }
    ...
    if(mod->functionList == NULL){
        fprintf(stderr, "in secmod_LoadPKCS11Module, before CK_FUNCTION_LIST_PTR mod->functionList is NULL\n");
    } else {
        fprintf(stderr, "in secmod_LoadPKCS11Module: before CK_FUNCTION_LIST_PTR mod->functionList is not NULL\n");
    }
    // now mod->functionList is still NULL
    ...
    // now mod->functionList is not NULL
    if(mod->functionList == NULL){
        fprintf(stderr, "in secmod_LoadPKCS11Module, after CK_FUNCTION_LIST_PTR, mod->functionList is NULL\n");
    } else {
        fprintf(stderr, "in secmod_LoadPKCS11Module, after CK_FUNCTION_LIST_PTR, mod->functionList is not NULL\n");
    }
#ifdef DEBUG_MODULE
    if (PR_TRUE) {
    ...
    if(mod->functionList){
        fprintf(stderr, "before secmod_ModuleInit, mod->functionList is not NULL\n");
    } else {
        fprintf(stderr, "before secmod_ModuleInit, mod->functionList is NULL\n");
    }
    ...
    if(mod->functionList){
        fprintf(stderr, "after secmod_ModuleInit, mod->functionList is not NULL\n");
    } else {
        fprintf(stderr, "after secmod_ModuleInit, mod->functionList is NULL\n");
    }
