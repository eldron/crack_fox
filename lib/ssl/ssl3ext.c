...
ssl3_HandleParsedExtensions(sslSocket *ss, SSLHandshakeType message)
{
    ...
    for (cursor = PR_NEXT_LINK(&ss->ssl3.hs.remoteExtensions);
        ...
        // // lijie:
        // if(extension->type == ssl_tls13_key_share_xtn){
        //     fprintf(stderr, "ssl3_HandleParsedExtensions: calling ssl_CallExtansionHandler to hanlde the key share extension\n");
        // }
...
ssl_ConstructExtensions(sslSocket *ss, sslBuffer *buf, SSLHandshakeType message)
{
    ...
    if(message == ssl_hs_client_hello){
        fprintf(stderr, "ssl_ConstructExtensions: type is client hello\n");
    } else {
        fprintf(stderr, "ssl_ConstructExtensions: type is not client hello\n");
    }

    switch (message) {
        case ssl_hs_client_hello:

            fprintf(stderr, "ssl_ConstructExtensions: handshake type is client hello\n");

            if (ss->vrange.max > SSL_LIBRARY_VERSION_3_0) {
                sender = clientHelloSendersTLS;

                fprintf(stderr, "set sender to clientHelloSendersTLS\n");
    ...
    for (; sender->ex_sender != NULL; ++sender) {
        ...
        if (ssl_FindCustomExtensionHooks(ss, sender->ex_type)) {
            //fprintf(stderr, "ex_type = %d, found extension hook\n", sender->ex_type);
            continue;
        } else {
            //fprintf(stderr, "ex_type = %d, not found extension hook\n", sender->ex_type);
        }
