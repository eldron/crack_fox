                    ...
                    CK_OBJECT_HANDLE_PTR phPrivateKey)
{
    ...
    switch (pMechanism->mechanism) {
        ...
        case CKM_EC_KEY_PAIR_GEN:
            ...
            fprintf(stderr, "in function NSC_GenerateKeyPair: case is CKM_EC_KEY_PAIR_GEN\n");
            ...
            fprintf(stderr, "NSC_GenerateKeyPair calling EC_NewKey\n");
            fprintf(stderr, "NSC_GenerateKeyPair: ecParams address is %llu\n", (unsigned long long) ecParams);

            rv = EC_NewKey(ecParams, &ecPriv);
            if (rv != SECSuccess) {
                fprintf(stderr, "NSC_GenerateKeyPair: EC_NewKey failed\n");
            ...
            fprintf(stderr, "NSC_GenerateKeyPair: EC_NewKey succeeded\n");
            if(ecParams->fieldID.type == ec_field_plain){
                fprintf(stderr, "NSC_GenerateKeyPair: ecParams->field.ID.type is ec_field_plain\n");
            } else if(ecParams->fieldID.type == ec_field_GFp){
                fprintf(stderr, "NSC_GenerateKeyPair: ecParams->fieldID.type is ec_field_GFp\n");
            } else {
                fprintf(stderr, "NSC_GenerateKeyPair: ecParams->fieldID.type is ec_field_GF2m\n");
            }
            ...
            if (PR_GetEnvSecure("NSS_USE_DECODED_CKA_EC_POINT") ||
                ...
                if(PR_GetEnvSecure("NSS_USE_DECODED_CKA_EC_POINT")){
                    fprintf(stderr, "NSC_GenerateKeyPair: NSS_USE_DECODED_CKA_EC_POINT\n");
                }
                if(ecParams->fieldID.type == ec_field_plain){
                    fprintf(stderr, "NSC_GenerateKeyPair: ecParams->fieldID.type is ec_field_plain\n");
                }

                PORT_FreeArena(ecParams->arena, PR_TRUE);

                fprintf(stderr, "NSC_GenerateKeyPair: sftk_AddAttributeType(publicKey, CKA_EC_POINT) is called\n");
